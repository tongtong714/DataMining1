...
public class Stat {
	public static void main(String[] args) throws Exception {
		...
		List<RawData> list = new FileReader().readFromFile("res/Analysis.txt");
		printSeason(list);
		printSize(list);
		printSpeed(list);
		printVal("mxPH", list);
	...
	private static void printSeason(List<RawData> list){
		int winter=0;
		int spring=0;
		int autumn=0;
		int summer=0;
	...
	private static void printSize(List<RawData> list){
		int small=0;
		int medium=0;
		int large=0;
		for(int i=0;i<list.size();i++){
	...
	private static void printSpeed(List<RawData> list){
		int low=0;
		int medium=0;
		int high=0;
		for(int i=0;i<list.size();i++){
	...
	private static void printVal(String name,List<RawData> list){
		double sum = 0;
		double max = 0;
		double min = 9999;
		List<Double> temp = new ArrayList<>();
		for(int i=0;i<list.size();i++){
			if(list.get(i).getIndex(name) != RawData.NO_EXIST){
